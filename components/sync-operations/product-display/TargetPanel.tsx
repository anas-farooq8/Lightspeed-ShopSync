...
import { Package, ExternalLink, RotateCcw, X, GripVertical } from 'lucide-react'
...
export function TargetPanel({
  ...
  const handleImageDragOver = (e: React.DragEvent, index: number) => {
    ...
    if (lastImageDropTargetRef.current === index) return
    lastImageDropTargetRef.current = index
    ...
    <Card className="border-border/50 flex flex-col h-fit overflow-hidden">
      ...
      <CardContent className="space-y-3 sm:space-y-4 pt-0 px-4 sm:px-6 pb-4 sm:pb-6">
        <div className="flex flex-row gap-3 sm:gap-5 min-w-0">
          ...
          <div className="flex-1 min-w-0 space-y-2 sm:space-y-3">
            <div className="grid grid-cols-2 gap-x-2 sm:gap-x-6 gap-y-2 sm:gap-y-4 text-[13px] sm:text-sm md:text-base">
            ...
            <div className="pt-2 flex items-center gap-2">
              <Label className="text-xs font-bold uppercase shrink-0">Visibility</Label>
              ...
              <Button
                ...
                size="sm"
                onClick={onResetShop}
                className="h-9 px-3 text-xs cursor-pointer shrink-0 ml-auto"
              >
      ...
      <Dialog open={imageDialogOpen} onOpenChange={setImageDialogOpen}>
        <DialogContent className="max-w-4xl">
          {targetProductImageUrl && (
            <img src={targetProductImageUrl} alt="Product" className="w-full h-auto" />
          )}
        </DialogContent>
      </Dialog>
    </Card>
